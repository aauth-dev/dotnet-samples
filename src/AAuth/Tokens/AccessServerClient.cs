...
/// response is not yet handled — the identity-claims push is tracked as a
/// later phase — and surfaces as a <see cref="NotSupportedException"/>.</para>
/// </remarks>
public sealed class AccessServerClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Deferred response: the AS needs interaction (consent) or claims
            // before it can issue an auth token.
            if (response.StatusCode == HttpStatusCode.Accepted)
            {
                // requirement=claims is a spec-mandated active claims PUSH
                // (POST directed sub + claims to Location), not the poll loop.
                // It is only meaningful once an AS actually requests claims and
                // is tracked as a later phase; surface it explicitly until then.
                var requirement = ExtractRequirementType(response);
                if (string.Equals(requirement, "claims", StringComparison.Ordinal))
                {
                    response.Dispose();
                    throw new NotSupportedException(
                        "Access Server returned 202 requirement=claims, but no claims handler is configured. "
                        + "Identity-claims push is not yet supported by AccessServerClient.");
                }
                ...
                try
                {
                    ...
                    response = await new DeferredPoller(_signedClient, request.PollerOptions)
                        .PollAsync(pendingUrl, cancellationToken).ConfigureAwait(false);
                }
                catch (TimeoutException ex)
                {
                    throw new AAuthInteractionTimeoutException(
                        $"Access Server deferred interaction did not complete within the polling budget: {ex.Message}",
                        ex);
                }
