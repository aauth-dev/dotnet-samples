...
// Phase 8 visual verification: for each flow, confirm the top actor bar shows
...
for (const { mode, server, url } of cases) {
  test(`actor bar shows ${server} for the ${mode} flow`, async ({ page }) => {
    ...
    await page.locator('header.topbar').screenshot({ path: `/tmp/phase8-bar-${server}.png` });
    await page.screenshot({ path: `/tmp/phase8-full-${server}.png`, fullPage: true });
  });
}
