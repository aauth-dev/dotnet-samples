...
test.describe('Call Chain (Guided Tour)', () => {
  ...
  test('two approvals replay through the orchestrator to a three-party 200', async ({ page, context }) => {
    await openTour(page);
    await selectFlow(page, TourMode.CallChain);

    await runAll(page);

    // Hop 1 — parked on the Agent → Orchestrator approval (6 steps done).
    ...
    // The background poll resolves the Orchestrator-audience auth_token (8 of
    // 13 steps done: user-approval + poll).
    await expect(doneSteps(page)).toHaveCount(8, { timeout: 120_000 });

    // "Run all" advances the hop-2 retry (the Orchestrator re-emits its own
    ...
    // The background poll of the Orchestrator pending URL resolves the chained
    ...
    // Step 12 ("Poll Orchestrator pending → 200") holds the combined result:
