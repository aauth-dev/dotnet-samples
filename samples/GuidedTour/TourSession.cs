...
public sealed class TourSession : IAsyncDisposable
{
    ...
    public bool IsIdentityMode =>
        ...
        || (!HasPersonServer && _mode != TourMode.Bootstrap && _mode != TourMode.SubAgent && _mode != TourMode.ResourceManaged && _mode != TourMode.RichRequest);
    ...
    public bool AwaitingUserApproval =>
        (IsDeferredMode || (IsFederatedMode && _federatedPending) || IsRichRequestMode || IsCallChainPending || IsMissionMode || IsMissionCallChainMode || IsResourceManagedMode)
    ...
    {
        if (!(IsDeferredMode || (IsFederatedMode && _federatedPending) || IsRichRequestMode || IsCallChainPending || IsMissionMode || IsMissionCallChainMode || IsResourceManagedMode)) { return Task.CompletedTask; }
    ...
    {
        if (!(IsDeferredMode || (IsFederatedMode && _federatedPending) || IsRichRequestMode || IsCallChainPending || IsMissionMode || IsMissionCallChainMode || IsResourceManagedMode) || _pendingUrl is null)
        {
            return Task.CompletedTask;
        }
