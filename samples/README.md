...
Fourteen sample applications demonstrating AAuth flows end-to-end. The six Aria
resource servers (Profile, Inbox, Calendar, Trips, Wallet, Bookings) live under
...
| [Bookings](MockResourceServers/Bookings/) | 5005 | Experimental Rich Resource Requests (R3) resource server — trip search, hold, and conditional booking proposal |
...
| [SampleApp](SampleApp/) | 5240 | Golden example — one page per signing mode plus call chain, federated four-party, resource-managed Inbox, and experimental R3 rich trip booking |
...
This starts Profile + Inbox + Calendar + Trips + Wallet + Bookings + Concierge + MockPersonServer + MockAgentProvider + MockAccessServer (stub + dedicated R3 AS) + GuidedTour + SampleApp in parallel, prints their URLs, and tears them down on `Ctrl+C`. Then open the **GuidedTour** at <http://localhost:5400> and click **Run all**, or the **SampleApp** at <http://localhost:5240>.
...
Simple Blazor app showing each signing mode as a separate page. Open <http://localhost:5240>. Requires the resource servers (Profile, Inbox, Calendar, Trips, Wallet, Bookings), MockPersonServer, Concierge, and the dedicated R3 Access Server on `:5501` for the Rich Trip Booking page. MockAgentProvider is needed only for the JWKS-URI enrollment page.
...
make resources       # only the six Aria resource servers (Profile :5000, Inbox :5004, Calendar :5001, Trips :5002, Wallet :5003, Bookings :5005)
